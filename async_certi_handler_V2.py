# -*- coding: utf-8 -*-
"""
Created on Wed Jun 30 03:09:19 2021
@author: abhigshr, product development intern 2021
Title: asynchronus certificate handler
Build : V1
"""
...
file_name_p = str(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H-%M-%S')) + "_cert_present" + ".csv"
file_name_a = str(datetime.datetime.fromtimestamp(time.time()).strftime('%Y-%m-%d_%H-%M-%S')) + "_cert_absent" + ".csv"
        ...
        for i in range(len(full_list)):
            if full_list[i] != -1:
                for j in range(len(full_list[i]["DeviceCertificates"])):
                    if full_list[i]["DeviceCertificates"][j]["Name"] == resj["Devices"][i]["UserId"]["Name"]:
                        fill_array(i, j, True)
                fill_array(i, j, False)
            else:
                pass
